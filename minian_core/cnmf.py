...
def update_temporal_block(
    ...
    **kwargs
    ...
    if use_smooth:
        YrA_ar = filt_fft(YrA, noise_freq, "low",fs=30.0)
...
def filt_butter(sig_2d: np.ndarray, btype: str, freq: float, fs: float) -> np.ndarray:
    """
    (正确版本) 对 (N, n_frames) 2D 数组应用 Butterworth 滤波器。
    
    - 正确使用 'fs' (采样率) 计算 Wn
    - 使用 'filtfilt' 进行零相位滤波 (对信号/噪声分离至关重要)
    - 使用 'axis=1' 进行高效的矢量化处理
    """
    
    # sig_2d 的形状是 (N, n_frames)
    ...
    try:
        ...
        print(f"[ERROR] 创建滤波器失败: Wn={Wn}, btype={btype}, fs={fs}, freq={freq}. {e}")
        return sig_2d # 失败时返回原信号    

    # 3. 沿时间轴 (axis=1) 应用零相位滤波
    try:
        return filtfilt(b, a, sig_2d, axis=0)
    except ValueError as e:
        # (例如，如果 n_frames 太短)
        print(f"[ERROR] filtfilt 应用失败: {e}. 数据形状: {sig_2d.shape}")
        return sig_2d # 失败时返回原信号

def filt_fft(sig_2d: np.ndarray, btype: str, freq: float, fs: float) -> np.ndarray:
    """
    (正确版本) 对 1D 数组 (n_frames,) 应用 FFT 滤波器。
    
    - 正确使用 'fs' (采样率) 来计算频率轴
    - 运行在由 xr.apply_ufunc 传入的 1D 数组上
    """
    
    # sig_2d 的形状是 (n_frames,)
    n_frames = sig_2d.shape[0] # 🔴 修正 1: (原为 shape[1])
    if n_frames == 0:
        return sig_2d

    # 1. 沿 axis=0 (n_frames 维度) 计算 rfft
    sig_rfft = numpy_fft.rfft(sig_2d, axis=0) # 🔴 修正 2: (原为 axis=1)
    
    # 2. 计算 rfft 对应的频率轴 (单位: Hz)
    # rfftfreq 的范围是 [0, fs/2]
    freq_axis = numpy_fft.rfftfreq(n_frames, d=1.0/fs)
    
    # 3. 确定要置零的索引
    if btype == "low":
        # 低通：保留低频 (<= freq)，去除高频 (> freq)
        zero_indices = np.where(freq_axis > freq)[0]
    elif btype == "high":
        # 高通：去除低频 (< freq)，保留高频 (>= freq)
        zero_indices = np.where(freq_axis < freq)[0]
    else:
        return sig_2d # 类型错误，返回原信号

    # 4. 应用掩码 (置零)
    if zero_indices.size > 0:
        # sig_rfft 是 1D 数组
        sig_rfft[zero_indices] = 0 # 🔴 修正 3: (原为 2D 的 np.put_along_axis)
    
    # 5. 计算逆 FFT
    result = numpy_fft.irfft(sig_rfft, n=n_frames, axis=0)
    return result.astype(np.float32)


# ======================================================================
# 步骤 2: (替换) 定义 Dask/Xarray 包装函数 'smooth_sig'
# ======================================================================

# 文件: cnmf.py
# 函数: smooth_sig (约 1630 行)

def smooth_sig(
    sig: xr.DataArray, freq: float, fs: float, method="butter", btype="low"
) -> xr.DataArray:
    """
    (正确版本) Dask/Xarray 包装器，用于并行执行滤波。
    
    - 必须传入 'fs' (采样率)。
    - 'filt_func' 必须映射到我们新定义的 vectorized_... 函数。
    """
    try:
        # 关键：映射到我们新定义的、正确的、
        # 能够处理 (N, n_frames) 2D 数组的函数
        filt_func = {
            "fft": filt_fft, 
            "butter": filt_butter
        }[method]
    except KeyError:
        raise NotImplementedError(f"不支持的方法: {method}。请使用 'fft' 或 'butter'。")

    # apply_ufunc 将会把 (H, W, T) 数组按块 (blockwise) 
    # 拆分成 (N, T) 的 2D 数组，
    # 然后传递给 filt_func (例如 filt_butter)
    sig_smth = xr.apply_ufunc(
        filt_func,
        sig,
        input_core_dims=[["frame"]],   # 指定输入的核心维度
        output_core_dims=[["frame"]],  # 指定输出的核心维度
        
        exclude_dims=set(),            
        
        vectorize=True,                # 关键：启用矢量化
        kwargs={"btype": btype, "freq": freq, "fs": fs}, # 关键：传递 fs
        dask="parallelized",           # 关键：启用 Dask 并行
        output_dtypes=[sig.dtype],     # 保持原始数据类型
    )
    return sig_smth
...
def graph_optimize_corr(
    ...
    # 🔴 FIX: Sort by 'source' *before* calculating 'grp'
    egd_diff = egd_diff.sort_values("source")
    grp = np.arange(len(egd_diff)) // step_size
    
    # Now groupby(grp) aligns with the sorted data
    for igrp, eg_sub in egd_diff.groupby(grp):
...
def smooth_corr(
    ...
    """
    Wraps around :func:`filt_fft` and :func:`idx_corr` to carry out both

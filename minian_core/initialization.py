...
def seeds_init(
    ...
    # 转换为xarray.DataArray
    seeds_arr = xr.DataArray(
        seeds[["height", "width", "seeds"]].values,
        dims=['index', 'features'],
        coords={
            'height': ('index', seeds['height'].values),
            'width': ('index', seeds['width'].values),
            'seeds': ('index', seeds['seeds'].values)
        }
    )
    return seeds_arr
...
def pnr_refine(
    ...
    pnr = xr.apply_ufunc(
        ...
        input_core_dims=[["frame"]],
        output_core_dims=[[]],
        kwargs={"freq": noise_freq, "q": q,"fs":fs},
        vectorize=True,
        dask="parallelized",
        output_dtypes=[float],
    ).compute()
...
def pnr_perseed(a: np.ndarray, freq: float, q: tuple, fs: float) -> float:
    """
    ...
    fs : float
        Sampling frequency (required for filt_fft).
    ...
    # 🔴 修正: 使用关键字参数调用，并传入 'fs'
    a = filt_fft(a, btype="high", freq=freq, fs=fs)
    
    ptp_noise = ptp_q(a, q)
    if ptp_noise == 0: # 避免除以零
        return 0.0
...
def seeds_merge(
    ...
    # 🔴 修正 1: 创建一个副本并重置索引
    # 这解决了 SettingWithCopyWarning 和 KeyError
    seeds_input_df = seeds.copy().reset_index(drop=True)
    
    print("computing distance")
    nng = radius_neighbors_graph(seeds_input_df[["height", "width"]], thres_dist)
    print("computing correlations")
    adj = adj_corr(varr, nng, seeds_input_df[["height", "width"]], noise_freq)
    print("merging seeds")
    adj = adj > thres_corr
    adj = adj + adj.T
    labels = label_connected(adj, only_connected=True)
    
    # 'iso' 现在包含的是 [0, 1, 2...] 范围内的行位置
    ...
    for cur_cmp in np.unique(labels):
        if cur_cmp < 0:
            continue
        cur_smp = np.where(labels == cur_cmp)[0] # 'cur_smp' 也是行位置
        cur_max = np.array(
            [
                max_proj.sel(
                    # 🔴 修正 2: 使用 .iloc (基于位置) 从重置索引的 df 中读取
                    height=seeds_input_df.iloc[s]["height"], width=seeds_input_df.iloc[s]["width"]
                )
                for s in cur_smp
            ]
        )
    ...
    # 🔴 修正 3: 在新的 DataFrame 上设置掩码
    seeds_input_df["mask_mrg"] = False
    
    # 'seeds_final' 包含的是行位置 (0, 1, 2...)
    # 'seeds_input_df' 的索引也是 (0, 1, 2...)
    # 因此 .loc 现在可以安全工作
    seeds_input_df.loc[list(seeds_final), "mask_mrg"] = True
    
    return seeds_input_df
